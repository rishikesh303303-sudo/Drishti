...
// 👈 PURANE Overview() FUNCTION KO ISSE REPLACE KARNA HAI:
function Overview({ prediction, loading }) {
  const [assigned, setAssigned] = useState(false);
  
  // Model data se risk score nikalna
  const riskScore = prediction ? prediction.risk_score : 0.78;
  const isFraud = prediction?.is_fraud === 1;
    ...
    <section className="overview glass">
      <div className="case-identity">
        ...
        <div>
          <div className="case-line">
            ...
            {/* 👈 Yahan risk badge dynamic ho jayega */}
            <span className={`risk-badge ${isFraud ? "danger" : ""}`}>
              {loading ? "ANALYZING..." : (isFraud ? "HIGH RISK / FRAUD" : "HIGH RISK")}
            </span>
          </div>
          <p>North Delhi Zone 14 • UPI Fraud • Filed: 2h ago</p>
        </div>
      </div>
      ...
      <div className="overview-metrics">
        <div>
          ...
          {/* 👈 Yahan model ka score dikhega */}
          <strong className="danger-number">
            {loading ? "..." : riskScore.toFixed(2)}
          </strong>
        </div>
        <div>
          <small>Confidence</small>
          <strong>{prediction ? `${(prediction.risk_score * 100).toFixed(0)}%` : "81%"}</strong>
        </div>
      ...
      <div className="assign-actions">
        ...
        <button className="btn-ghost">
          <Icon name="users" size={15} /> Reassign
        </button>
      </div>
    </section>
  );
...
function VersionHistory() {
 ...
 function CaseDetail({ caseData }) {
  // 1. NAYA ADD KIYA: Model prediction aur loading states
  const [prediction, setPrediction] = useState(null);
  const [loading, setLoading] = useState(false);

  // 2. NAYA ADD KIYA: Backend FastAPI model (/api/predict-fraud) ko call karne ka function
  const runModelPrediction = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/api/predict-fraud`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          transaction_id: caseData?.id || "DRS-45821",
          amount: caseData?.amount || 50000,
          upi_handle: caseData?.upi || "fraud@paytm"
        }),
      });

      const result = await response.json();
      setPrediction(result); // Model ka output state mein save hoga
    } catch (error) {
  ...
  // 3. NAYA ADD KIYA: Page load hote hi model run karne ke liye
  useEffect(() => {
    runModelPrediction();
  }, []);
  ...
  return (
    <>
      ...
      <main className="app-shell">
        ...
        {/* Yahan aap chahein toh prediction data ko Overview component mein props ke taur par pass kar sakte hain */}
        <Overview prediction={prediction} loading={loading} onReRun={runModelPrediction} />
